...
class HealthBar {
    ...
    public HealthBar(GameContext ctx) {
        context = ctx;
        final Creature playerCreature = context.getPlayerCreature();
        max = playerCreature.healthPoints;
    ...
    public void paint(Graphics g) {
        ...
        int barwidth = value * (width - 2 * (borderSize + marginSize)) / max;
        g.fillRect(
                x + borderSize + marginSize, y + borderSize + marginSize,
                barwidth, height - 2 * (borderSize + marginSize));
    }

    void updateHealth(Graphics graphics) {
        final Creature playerCreature = context.getPlayerCreature();
